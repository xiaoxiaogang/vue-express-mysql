...
const branchName= 'master';  //需要提交的分支名称
const modifyText = '"自动提交编译文件，时间：'+(new Date()).toLocaleString()+'"';
...
function myFirstFunction(callback) {
    ...
    if(ret.stdout.indexOf('Already on') > -1 || ret.stderr.indexOf('Already on') > -1 || ret.stdout.indexOf('Your branch is up to date') > -1){
        callback(null, '1');
    } else {
        callback(null, '0');
    }
...
function mySecondFunction(arg1, callback) {
    if(arg1 === '0'){
        console.log('checkout失败，退出checkout');
        shell.exit(0);
    }
